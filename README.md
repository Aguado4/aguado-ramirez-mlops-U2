...
Dados los avances tecnológicos en medicina, la cantidad de información que existe de los pacientes es abundante. Sin embargo, para algunas enfermedades no tan comunes (huérfanas), los datos escasean. Se requiere construir un modelo capaz de predecir, a partir de los síntomas reportados, si un paciente podría sufrir alguna enfermedad, cubriendo tanto enfermedades comunes (muchos datos) como huérfanas (pocos datos).

Para este ejercicio el modelo se simula con una función determinística que recibe tres parámetros del paciente y retorna uno de los siguientes estados:
